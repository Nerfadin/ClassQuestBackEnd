...
const HouseErrors_1 = require("./HouseErrors");
const HouseFirebaseAdapter_1 = require("./HouseFirebaseAdapter");
let HouseService = class HouseService {
    ...
    async createHouse(playerId) {
        ...
        if (!house) {
            const houseCreated = await this.houseDao
                ...
                .catch((error) => {
                throw HouseErrors_1.HouseErrors.HouseCreateError(error);
            });
            return houseCreated;
        }
        return house;
    ...
    async getHouse(playerId) {
        const house = await this.houseDao.getHouse(playerId).catch((error) => {
            throw HouseErrors_1.HouseErrors.GetHouseError(error);
        });
        ;
        return house;
    }
...
HouseService = __decorate([
    ...
    __param(0, (0, tsyringe_1.Inject)(() => HouseFirebaseAdapter_1.HouseFirebaseAdapter)),
    __metadata("design:paramtypes", [HouseFirebaseAdapter_1.HouseFirebaseAdapter])
], HouseService);
